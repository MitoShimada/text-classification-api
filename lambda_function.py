...
def lambda_handler(event, context):
    ...
    print(message)
    result = evaluate(_default(message))
    dynamodb = boto3.resource('dynamodb')
    # 保存するResultが一位になるようインクリメント
    sq_table = dynamodb.Table('Sequences')
    id_resp = sq_table.update_item(
        Key={
            'name': 'EvalResults'
        },
    ...
    # 判定結果の保存
    unique_id = int(id_resp["Attributes"]["current_number"])
    eval_result_table = dynamodb.Table('EvalResults')
    eval_result_table.put_item(
        Item={
            'id': unique_id,
    ...
    response = {
        ...
        "body": json.dumps({'result': result, 'id': unique_id}, ensure_ascii=False),
...
def evaluate(args):
    ...
    print(output.data)
    print("hyp: %d" % np.argmax(output.data))   # output.dataの最大値を返す
    return {"result": int(np.argmax(output.data)), "data": output.data.tolist()}
